...
#
nameoffun <- "sphere" ## objective function, "quadratic", 
...
#
nameofoptim <- "normal_search" # "random_search" , "normal_search" , "lbfgs", "ego" , "cmaes", "NS_lbfgs"
...
if (nameofoptim=="random_search") {
  ...
  param <- list(LB=zLB,UB = zUB,budget = budget,dim=zdim, xinit=rep(-4,zdim),sigma=10) # param for normal_search
  RandomInit <- TRUE 
